...
def get_unique_signups_for_interval(database, start_date_epoch, end_date_epoch, interval_seconds, show_user_ids):
    sql = """
    SELECT time DIV %s AS interval_time,
...
def lambda_handler(event, context):
    try:
        ...
        show_none = body.get('show_none', False)
        ...
        with Database() as database:
            result_data = get_unique_signups_for_interval(database, epoch_start_time, epoch_end_time, interval_seconds, show_user_ids)
    ...
    return {
        'statusCode': 200,
        'body': json.dumps({
            'unique_signups_counts': result_data
        })
    }

...
def check_ticket_been_transferred_from_real_ticket_id(real_ticket_id):
    try:
    ...
    with connection.cursor() as cursor:
        ...
        username = result[0]
        password = result[1]
        ticket_reference = result[2]

        with requests.session() as s:

            auth_token = login(s, username, password)

            return check_ticket_been_transferred(s, auth_token, ticket_reference)


def check_ticket_been_transferred(session, auth_token, ticket_reference):
    get_bookings_url = "https://api.fixr.co/api/v2/app/booking?only=future"
    headers = {
        "Authorization": "Token " + auth_token
    }

    response = session.get(get_bookings_url, headers=headers)
    for ticket in response.json()['data']:
        if ticket_reference == ticket['reference_id']:
            return False
